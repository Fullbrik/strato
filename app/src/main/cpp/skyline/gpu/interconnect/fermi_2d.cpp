...
namespace skyline::gpu::interconnect {
    ...
    std::pair<Fermi2D::FermiTexture, bool> Fermi2D::GetFermiTexture(const Surface &surface, u32 oobReadStart, u32 oobReadWidth) {
        ...
        if (surface.memoryLayout == MemoryLayout::Pitch) {
            dimensions = gpu::texture::Dimensions{surface.pitch / format->bpb, surface.height, 1};
    ...
    void Fermi2D::Blit(const Surface &srcSurface, const Surface &dstSurface, float srcRectX, float srcRectY, u32 dstRectWidth, u32 dstRectHeight, u32 dstRectX, u32 dstRectY, float duDx, float dvDy, SampleModeOrigin sampleOrigin, bool resolve, SampleModeFilter filter) {
        ...
        auto srcTextureView{gpu.texture.FindOrCreate({
            .tag = executor.executionTag,
            .mappings = std::move(srcFermiTexture.mappings),
            .sampleDimensions = srcFermiTexture.dimensions,
            .tileConfig = srcFermiTexture.tileConfig,
            .layerStride = srcFermiTexture.layerStride,
            .viewFormat = srcFermiTexture.format,
            .viewAspect = srcFermiTexture.format->vkAspect
        })};

        auto dstTextureView{gpu.texture.FindOrCreate({
            .tag = executor.executionTag,
            .mappings = std::move(dstFermiTexture.mappings),
            .sampleDimensions = dstFermiTexture.dimensions,
            .tileConfig = dstFermiTexture.tileConfig,
            .layerStride = dstFermiTexture.layerStride,
            .viewFormat = dstFermiTexture.format,
        ...
        gpu.helperShaders.blitHelperShader.Blit(
            gpu,
            {
                .width = duDx * static_cast<float>(dstRectWidth),
                .height = dvDy * static_cast<float>(dstRectHeight),
                .x = centredSrcRectX,
                .y = centredSrcRectY,
            },
            {
            ...
            [&, this](auto &&executionCallback) {
                std::vector<std::pair<HostTextureView *, TextureSyncRequestArgs>> sampledImage{{srcTextureView, {
                    .isRead = true,
                    .isWritten = false,

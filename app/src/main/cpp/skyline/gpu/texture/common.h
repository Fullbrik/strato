...
namespace skyline::gpu::texture {
    ...
    constexpr Dimensions CalculateBaseDimensions(Dimensions dimensions, MsaaConfig msaa) {
        u32 msaaHeight{[msaa]() -> u32 {
            switch (msaa) {
                case MsaaConfig::e1x1:
                    return 1;
    ...
    struct FormatBase {
        ...
        constexpr Dimensions GetDimensionsInBytes(Dimensions dimensions) const {
            return {util::DivideCeil<u32>(dimensions.width, blockWidth) * bpb, util::DivideCeil<u32>(dimensions.height, blockHeight), dimensions.depth};
        }

        constexpr Dimensions GetDimensionsFromBytes(Dimensions byteDimensions) const {
            return {util::DivideCeil<u32>(byteDimensions.width, bpb) * blockWidth, byteDimensions.height * blockHeight, byteDimensions.depth};
        }
         ...
         */
        constexpr vk::ImageAspectFlags Aspect(vk::ComponentMapping swizzleMappings) const {
            bool first{};
            if (swizzleMappings.r == vk::ComponentSwizzle::eR || swizzleMappings.g == vk::ComponentSwizzle::eR || swizzleMappings.b == vk::ComponentSwizzle::eR || swizzleMappings.a == vk::ComponentSwizzle::eR)
                first = true;

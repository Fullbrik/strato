            ...
            if (!*gpu.state.settings->useDirectMemoryImport) {
                if (srcBuffer->dirtyState == Buffer::DirtyState::GpuDirty) {
                    ...
                    if (srcBuffer->dirtyState == Buffer::DirtyState::CpuDirty)
                        LOGE("Buffer ({} - {}) is marked as CPU dirty while CPU backing writes are blocked, this is not valid", fmt::ptr(srcBuffer->guest->begin().base()), fmt::ptr(srcBuffer->guest->end().base()));
